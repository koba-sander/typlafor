/**
 * このコードは、条件分岐を行うための関数 `when` を定義しています。 `when` 関数は、
 * 数値 `value` を受け取り、以下の2つのメソッドを持つオブジェクトを返します。
 * 1. `on` メソッド：条件分岐を行います。引数として、条件を判定するため
 * の関数 `predicate` と、条件が真であった場合に実行する関数 `fn` を取ります。
 *  `on` メソッドは、条件が真の場合、オブジェクトを返します。このオブジェクトには、
 * `on` メソッドが再度呼び出されることで、新しい条件分岐を追加することができます。
 * また、条件が偽であった場合、`otherwise` メソッドを呼び出します。
 *
 * 2. `otherwise` メソッド：引数として、条件分岐を追加しなかった場合に実行する
 * 関数 `fn` を取ります。このメソッドは、常に引数で渡された関数を実行し、その
 * 戻り値を返します。`on` メソッドと `otherwise` メソッドの両方がチェーン可能で
 * あり、同じ `value` を引き継ぎます。 `when` 関数を使用すると、単純な条件分岐を
 * より読みやすく、チェーン可能な方法で表現できます。
 *
...
export function switcher1(hoge) {
    const result = when(hoge)
        .on((v) => v === 1, () => "A")
        .on((v) => v === 2, () => true)
        .on((v) => v === 3, () => ["aaa", "bbb"])
